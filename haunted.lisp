...
(defvar *ghosts-active*    nil "Ghosts active")
(defvar *ghosts-disabled*  nil "Ghosts disabled")
...
(defun handle-use (loc item default)
  (cond ((and (eq item 'vacuum)
         ...
         (cond ((not (item-in-backpack-p 'batteries))
               ...
               ((and (eq loc 'cobwebby-room)
                     *ghosts-active*)
                (setf *ghosts-active* nil)
                (setf *ghosts-disabled* t)
    ...
    (handler-case
        (with-open-file (out name :direction :output :if-exists :supersede)
          (let ((alist `((item-locations ,@(hash-to-alist *item-locations*))
                         ...
                         (ghosts-active ,*ghosts-active*)
                         (ghosts-disabled ,*ghosts-disabled*)
    ...
    (handler-case
        (with-open-file (in name :direction :input)
          (let ((alist (read in)))
            ...
            (setf *ghosts-active* (cadr (assoc 'ghosts-active alist)))
            (setf *ghosts-disabled* (cadr (assoc 'ghosts-disabled alist)))
